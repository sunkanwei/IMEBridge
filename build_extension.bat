...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip = '%ZIP_PATH%'; $archive = [IO.Compression.ZipFile]::OpenRead($zip); try { $bad = @($archive.Entries | Where-Object { $_.FullName -match '(^|/)README\.md$|(^|/)build_extension\.(bat|command|sh)$|(^|/)\.git(/|$)|(^|/)\.serena(/|$)|(^|/)\.DS_Store$|__pycache__|\.pyc$|\.(bat|command|sh)$' } | ForEach-Object { $_.FullName }); if ($bad.Count -gt 0) { Write-Host 'ERROR: package contains excluded files:'; $bad | ForEach-Object { Write-Host ('  ' + $_) }; exit 1 } } finally { $archive.Dispose() }"
if errorlevel 1 goto :fail
